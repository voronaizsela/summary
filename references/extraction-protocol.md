...
| `contradiction` | places where the document conflicts with itself | both versions recorded, neither resolved |
...
- **Totals go in as stated, never recomputed.** If a column does not sum to its printed total, record that as a `contradiction`. It is a finding about the document, not an error to silently repair.
...
If the cross-referenced material is not present in what you were given, that is a gap — record it and surface it in the delivered text, in words.

## Multilingual documents

Quotes stay in the source language. Subject and note are in the output language. Terms with an established translation still get the original in parentheses at first use. Where a document exists in two languages and they differ, that is a `contradiction`, and often a significant one.
