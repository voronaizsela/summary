...
8. **Business register.** The output reads as an analytical note for a manager or a lawyer: neutral third person, precise business and legal vocabulary, headings as noun phrases («Порядок оплаты», «Ответственность сторон»). No colloquialisms («по сути», «деньги идут», «платит»), no questions or conversational phrasing in headings, no addressing the reader, no emphasis words, no metaphors — and no officialese overload either. Details and examples in `references/output-templates.md`, "Register". The same applies to follow-up answers.
9. **Output language follows the document**, unless the user asks otherwise or clearly writes in another language. Every heading, table header and label in that language; quotations stay in the source language.
10. **Coverage is exhaustive.** Every segment leaves a trace in the ledger.
11. **No voice of your own.** No assessment, advice or conclusion the document did not draw; no evaluative adjectives. If a term is dangerous, state it completely with its consequence.
12. **Exhaustive coverage, selective output.** The ledger holds everything; the summary holds what the reader needs to understand the document and act on it. The rest stays available for follow-up questions.
...
- **Sequence follows the logic of the subject, not the order of the text and not the order of extraction.** A contract is read as the life of the deal: scope and stages → payment terms → acceptance → obligations → liability → termination. A report: results → factors stated by the company → qualifications → plans. Per-type sequences are in `references/output-templates.md`.
- **Each section opens with prose that explains its mechanism**: one to four connected sentences saying how this part of the subject works according to the document. «Оплата производится в два этапа: авансовый платёж после подписания договора и окончательный расчёт после приёмки работ.» — then the table gives the amounts and deadlines.
...
1. **`## Резюме`** (in the output language) — one paragraph of 3–5 sentences. What the document is, who issued it or who the parties are, what it covers, and what it comes down to: the central result or the central deal, with at most two headline figures. Someone who reads only this knows what the document is and what it establishes.
...
3. **Source limitation line** — only if needed (scan, missing pages, attachment not supplied), one line directly after "Резюме".
...
5. **Write "Резюме" last**, from the rows that carry the whole document.

Rules for the prose:

- Entities, figures, units, dates and modality come from the rows unchanged. "Plans to" is not "will". "May" is not "shall".
- Connectives: sequence and structure ("после подписания акта", "для каждого этапа", "со стороны заказчика") are always available; cause, purpose and consequence only with a `link` row, and attributed where the document attributes («компания связывает это с…», «согласно условиям договора»).
...
- Can I say what the document is and what it establishes after reading "Резюме"? If not, rewrite it.
...
- Does any sentence or heading sound conversational — a colloquial word, a question-heading, an address to the reader? Rewrite it in business register.
...
Deliver "Резюме" and the sections. Nothing after them. Keep the verification record in working notes with real numbers; give them only if asked, and never a plausible invented count.
...
- **Give the reference.** End with the location in the document's own numbering: «Основание: п. 7.3 договора», «Приложение № 2, таблица «График платежей»». In follow-up answers this is useful, not clutter.
- **Short and in business register.** A few sentences of prose; a small table only if the answer is several comparable figures. No headings, no re-summary, no repetition of the summary.
...
Facts about what the user supplied, stated once in one line after "Резюме":
...
- **Depth** — "Резюме" only; the standard shape; or an exhaustive breakdown with fuller sections. The sequence and the verification do not change.
- **Focus** — if the user asks about specific things (risks, obligations, deadlines), "Резюме" stays, and only the relevant sections follow, in the same logical order.
